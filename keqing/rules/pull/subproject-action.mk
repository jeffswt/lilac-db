...
__PULL__SUBPROJECT_ACTION = "$(BIN_BASH)" "$(RULES)/pull/subproject-action.sh" "$(ACTION_DEPENDS)" "$(ARTIFACTS)" "$(BIN_CAT)" "$(BIN_CP)" "$(BIN_ECHO)" "$(BIN_GIT)" "$(BIN_MAKE)" "$(BIN_MKDIR)" "$(BIN_READLINK)" "$(BIN_RM)" "$(BIN_TOUCH)" "$(RULES)"
